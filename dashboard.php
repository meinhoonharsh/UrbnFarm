...
<div id="overall" class="w3-container city" style="display:none">
      ...
      <a href="reset.php?id=0"" onclick="return confirm('Really want to REset the Data')" class="btn btn-primary">Reset</a>
      <a href="logout.php" class="btn btn-primary">Logout</a>
      <hr>
  <table class="w3-table w3-striped">
    <tr>
   ...
   <?php
    ...
    if (mysqli_num_rows($resultdemand) > 0) {
      ...
      <th>'.$n.'</th>
      <td>'.$row['name'].'</td>
      <td>&#8377;'.$row['demand']*$row['ourprice'].'</td>
      <td>'.$row['demand'].'</td>
    </tr>';
    $totalPrice+=$row['demand']*$row['ourprice'];
    $totalWeight+=$row['demand'];
    $n+=1;
